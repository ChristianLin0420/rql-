"""
DQL (v11): one-step drift actor + ACTION-SHARP critic (IQL Q + bounded in-support contrast).

v10 post-mortem (measured): the geometric interpolation-value trains V(s, x_tau, tau) -> y for random x
at every tau<1, but y is independent of x -> the net learns to IGNORE the action (rank_acc~0.55,
act/state spread ratio ~0.02, and WORSENING). A flat critic gives the policy no gradient -> 0.0 success.

v11 fix (keep what was healthy: no overestimation / no collapse / no drift; one-step deploy):
  - Genuine action value Q(s,a) with the IQL transition target Q(s,a) <- r + gamma^h V(s')  ->
    action-informative by construction (target is transition-specific).
  - BOUNDED in-support contrast: relu( Q(s,a-) - Q(s,a) + m ) over negatives a- in {random, generator}
    -> lifts rank_acc AND pushes OOD Q down (so Q-ascent cannot run away into OOD).
  - Noise-free state value V(s) as its own head: V(s) <- expectile_kappa(Q(s,a_data)).
  - Trust-region one-step drift actor (from v10) now ascends the sharp Q. Deploy = 1 pass (medoid).
Leading gate: probe/rank_acc must climb (>=0.8) and probe/ratio must grow BEFORE the eval lands.
"""
import copy
...
class DQLv11Agent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
...
def get_config():
    return mlc.ConfigDict(dict(
        ...
        q_coef=1.0,          # bounded Q-ascent weight
