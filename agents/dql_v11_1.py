"""
DQL (v11.1): DRIFT-BASED actor (kept) + ACTION-SHARP critic (IQL Q + bounded in-support contrast),
with a stronger, more stable drift actor. v11 fixed the critic (rank_acc->1.0) but the drift actor was
weak+unstable (antmaze ~0.30, bouncing 0.5<->0.0). v11.1 targets exactly that, WITHOUT leaving the
drift paradigm:
  (1) ADVANTAGE-weighted drift: attraction weights = softmax((Q(s,a_cand) - V(s)) / adv_temp), peaked
      toward the argmax-advantage in-support data action (sharper improvement, not the blurred Q-mean).
      Still a drift toward data-derived targets (multimodal via per-sample kernel + repulsion).
  (2) DAMPED Q-ascent: q_coef 1.0 -> 0.25 on the lagged target_q, so the stable in-support advantage
      drift dominates and the ascent can't oscillate the policy.
  (3) ACTOR EMA at eval: deploy from a slow EMA of the generator (the RQL paper's own stability trick,
      which v11 lacked) -> removes the eval-to-eval swing.
Critic unchanged from v11 (IQL Q + contrast + noise-free V). Validate on antmaze vs v11's 0.30 first.
"""
import copy
...
class DQLv11_1Agent(flax.struct.PyTreeNode):
    rng: Any
    network: Any
...
def get_config():
    return mlc.ConfigDict(dict(
        ...
        n_gen=16, n_cand=32, state_bw=0.15, tau_scale=0.5, drift_step=1.0,
        adv_temp=0.5,        # advantage softmax temp (lower = sharper toward argmax-advantage action)
