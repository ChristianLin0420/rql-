...
CATS = [
    # (category, token, h, expectile, rho, discount, sparse, state_bw, bw_ps, dm)
    # bw_ps/dm (v11.4b, humanoidmaze only): per-state locality normalizer (batch-mean bw
    # masks 4x neighbor-density spread -> corridor states over-borrow, w_self p10 0.07)
    # and medoid deployment (rho=0 makes argmax-Q pure noise-outlier picking there).
    ...
    ("antmaze-large",        "navigate", 1, 0.7, 0.5, 0.99,  0, 0.5,  0, 0),
    ("antmaze-giant",        "navigate", 1, 0.7, 0.5, 0.995, 0, 0.5,  0, 0),
    ("humanoidmaze-medium",  "navigate", 1, 0.7, 0.0, 0.995, 0, 0.25, 1, 1),
    ("humanoidmaze-large",   "navigate", 1, 0.7, 0.0, 0.995, 0, 0.25, 1, 1),
    ("scene",                "play",     5, 0.7, 0.5, 0.99,  1, 1.0,  0, 0),
    ("puzzle-3x3",           "play",     5, 0.7, 0.5, 0.99,  1, 1.0,  0, 0),
    ...
    ("puzzle-4x4",           "play",     5, 0.9, 0.5, 0.99,  0, 1.0,  0, 0),
    ("cube-double",          "play",     5, 0.9, 0.5, 0.99,  0, 1.0,  0, 0),
    ("cube-triple",          "play",     5, 0.9, 0.5, 0.99,  0, 1.0,  0, 0),
    ("cube-quadruple",       "play",     5, 0.9, 0.5, 0.99,  0, 1.0,  0, 0),
]
rows=[]
for cat,tok,h,e,rho,disc,sp,bw,bps,dm in CATS:
    for t in range(1,6):
        env=f"{cat}-{tok}-singletask-task{t}-v0"
        rows.append((env,h,e,rho,disc,sp,bw,bps,dm))
with open("slurm/tasks.tsv","w") as f:
    f.write("env_name\th\texpectile\trho\tdiscount\tsparse\tstate_bw\tbw_ps\tdm\n")
