...
class ActorVectorField(nn.Module):
    ...
    @nn.compact
    def __call__(self, observations, actions, times=None, steps=None, is_encoded=False, *, train=True):
        ...
        inputs = jnp.concatenate([observations, actions] + ([times] if times is not None else []) + ([steps] if steps is not None else []), axis=-1)
